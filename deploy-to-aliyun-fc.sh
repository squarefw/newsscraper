...
main() {
    ...
    # 验证阿里云连接（尝试 eu-west-1，若失败则继续并在后续提示中更改）
    log_info "🔍 验证阿里云连接..."
    ...
    # API密钥配置 - 先选择源文件（优先仓库根的 api-keys.json），再将其复制到解压后的部署目录并在部署包内使用副本
    SOURCE_KEY_FILE="$PWD/config/api-keys.json"
    if [ ! -f "$SOURCE_KEY_FILE" ]; then
        if [ -f "../config/api-keys.local.json" ]; then
            SOURCE_KEY_FILE="../config/api-keys.local.json"
        elif [ -f "../config/api-keys.json" ]; then
            SOURCE_KEY_FILE="../config/api-keys.json"
        else
    ...
    for f in "../utils/sourceAnalyzer_*.js"; do
        if [ -f "$f" ]; then
            base=$(basename "$f")
    ...
    # WordPress配置
    echo ""
    log_warn "请配置WordPress连接信息 (config/config.remote-aliyun.json):"

    WP_URL=$(read_input "WordPress站点URL" "")
    if [ -n "$WP_URL" ]; then
        sed -i.bak "s|\"url\": \"https://your-wordpress-site.com\"|\"url\": \"$WP_URL\"|" config/config.remote-aliyun.json
    fi

    WP_USERNAME=$(read_input "WordPress用户名" "")
    if [ -n "$WP_USERNAME" ]; then
        sed -i.bak "s/\"username\": \"your-username\"/\"username\": \"$WP_USERNAME\"/" config/config.remote-aliyun.json
    fi

    WP_PASSWORD=$(read_input "WordPress密码" "")
    if [ -n "$WP_PASSWORD" ]; then
        sed -i.bak "s/\"password\": \"your-password\"/\"password\": \"$WP_PASSWORD\"/" config/config.remote-aliyun.json
    fi

    # AI提供商配置
    AI_PROVIDER=$(read_input "AI提供商 (openai/deepseek/anthropic)" "openai")
    sed -i.bak "s/\"provider\": \"openai\"/\"provider\": \"$AI_PROVIDER\"/" config/config.remote-aliyun.json

    log_success "WordPress和AI配置完成"

    # RSS源配置
    echo ""
    log_warn "配置RSS源 (config/targets.json):"
...
# 执行主函数
main "$@"
<parameter name="filePath">/Users/weifang/Sites/i0086/site/newsscraper/deploy-to-aliyun-fc.sh
